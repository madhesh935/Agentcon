...
from schemas import PMIRequest, PMIResponse, SecurePMIRequest, SecurePMIResponse, SecurityField
from security import decrypt_json, encrypt_json, require_key
from train_model import train as run_training, MODEL_PATH, NUMERIC_FEATURES, CATEGORICAL_FEATURES
...
def _run_pmi_prediction(data_dict: dict) -> PMIResponse:
    if model is None:
        raise HTTPException(
            status_code=503,
    ...
    return PMIResponse.from_prediction(
...
async def predict_pmi(request: PMIRequest):
    ...
    try:
        return _run_pmi_prediction(request.forensic_features())
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Prediction error: {e}") from e


@router.post("/predict/secure", response_model=SecurePMIResponse)
async def predict_pmi_secure(request: SecurePMIRequest):
    """PMI predict with AES-256-GCM encrypted request and response."""
    try:
        require_key()
    except ValueError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    if not request.security.ciphertext:
        raise HTTPException(status_code=400, detail="security.ciphertext is required.")
    try:
        features = decrypt_json(request.security.ciphertext)
        result = _run_pmi_prediction(PMIRequest.model_validate(features).forensic_features())
        payload = {
            "predicted_pmi_hours": result.predicted_pmi_hours,
            "confidence_score": result.confidence_score,
            "explanation": result.explanation,
            "message": result.message,
        }
        return SecurePMIResponse(
            security=SecurityField(ciphertext=encrypt_json(payload)),
        )
    except HTTPException:
        raise
        ...
        raise HTTPException(status_code=400, detail=f"Secure prediction error: {e}") from e
