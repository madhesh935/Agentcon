...
// Center between TN and KA
const TN_KA_CENTER: [number, number] = [12.5, 77.4];
const TN_KA_ZOOM = 6;
...
export function RiskMap() {
  ...
  useEffect(() => {
    ...
    zones.forEach((z) => {
      ...
      const stateLabel = (z as any).state === "KA" ? "Karnataka" : "Tamil Nadu";
      m.bindTooltip(
        `<div style="font-size:11px"><b>${z.district}</b> <span style="opacity:0.7">(${stateLabel})</span><br/>Crimes: ${z.crimes}<br/>Risk: ${z.risk}<br/>Officers: ${z.officers}</div>`,
        { direction: "top" },
      );
    });
    ...
    if (zones.length === 1) {
      ...
      map.setView(TN_KA_CENTER, TN_KA_ZOOM);
    }
  ...
  const labelDistrict = districtFilter ?? "Tamil Nadu · Karnataka";
